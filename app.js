...
import { View, Text, StyleSheet, TouchableOpacity, TextInput, Switch, ScrollView, StatusBar, Modal } from 'react-native';

const SERVERS = [
  { n: 'ألمانيا', c: 'DE', f: '🇩🇪', ip: '1.1.1.1' }, 
  { n: 'فرنسا', c: 'FR', f: '🇫🇷', ip: '2.2.2.2' },
  { n: 'أمريكا', c: 'US', f: '🇺🇸', ip: '3.3.3.3' }, 
  { n: 'الإمارات', c: 'UAE', f: '🇦🇪', ip: '4.4.4.4' }, 
  { n: 'اليابان', c: 'JP', f: '🇯🇵', ip: '5.5.5.5' }
];
...
export default function App() {
  ...
  const [sni, setSni] = useState('://whatsapp.com'); 
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.bg }]}>
          ...
          <View>
            ...
            <View style={[styles.statsRow, { backgroundColor: theme.card, borderColor: theme.border }]}>
              <View style={styles.statBox}>
                ...
                <Text style={styles.statLbl}>الوقت المتبقي ⏱️</Text>
              </View>
            </View>
            
            <TouchableOpacity style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border }]} onPress={() => setTab('servers')}>
              <Text style={styles.val}>{SERVERS[sIdx].f} {SERVERS[sIdx].n} ({SERVERS[sIdx].c})</Text>
              <Text style={styles.lbl}>موقع النفق 🌐</Text>
            </TouchableOpacity>
            
            <TouchableOpacity style={[styles.card, { backgroundColor: theme.card, borderColor: '#00D9A3' }]} onPress={() => setShowSSL(true)}>
              <Text style={[styles.val, { color: '#00D9A3' }]}>{port} : {sni}</Text>
              <Text style={styles.lbl}>إعدادات SSL المتقدمة ⚙️</Text>
            </TouchableOpacity>
          ...
          <View>
            {SERVERS.map((srv, idx) => (
              <TouchableOpacity key={idx} style={[styles.card, { backgroundColor: theme.card, borderColor: idx === sIdx ? '#00D9A3' : theme.border }]} onPress={() => { setSIdx(idx); setTab('home'); }}>
                <Text style={{ color: '#00D9A3', fontWeight: 'bold' }}>{srv.c}</Text>
                <Text style={styles.val}>{srv.f} {srv.n}</Text>
              </TouchableOpacity>
            ))}
          </View>
        )}
      ...
      <Modal visible={showSSL} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <View style={styles.btnRow}>
              <TouchableOpacity onPress={() => setShowSSL(false)}><Text style={styles.btnTxt}>CANCEL</Text></TouchableOpacity>
              <TouchableOpacity onPress={() => setShowSSL(false)}><Text style={[styles.btnTxt, { color: '#00D9A3' }]}>OK</Text></TouchableOpacity>
            </View>
          </View>
        </View>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  val: { fontSize: 14, fontWeight: 'bold', color: '#F8FAFC' },
  connBtn: { padding: 16, borderRadius: 23, alignItems: 'center', justifyContent: 'center', marginTop: 15, width: '85%', alignSelf: 'center' },
  bOff: { backgroundColor: '#1E253B' }, 
  bWait: { backgroundColor: '#B45309' }, 
  bOn: { backgroundColor: '#00D9A3' }, 
  connText: { color: '#FFF', fontWeight: 'bold', fontSize: 14 },
  ...
  statsRow: { padding: 12, borderRadius: 12, borderWidth: 1, marginBottom: 15 },
  statBox: { alignItems: 'center' }, 
  statVal: { fontSize: 22, fontWeight: 'bold' }, 
  statLbl: { fontSize: 11, color: '#94A3B8', marginTop: 3 },
  modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'center', alignItems: 'center' },
  modalCard: { backgroundColor: '#FFFFFF', width: '85%', borderRadius: 8, padding: 20 }, 
  modalTitle: { fontSize: 20, fontWeight: 'bold', color: '#333', marginBottom: 15 },
  row: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginVertical: 8 }, 
  popTxt: { fontSize: 14, color: '#333' },
  inputGroup: { borderBottomWidth: 1, borderBottomColor: '#CCC', marginVertical: 6, paddingVertical: 2 }, 
